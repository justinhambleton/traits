...
- Date: 2026-02-15
- Decision: Implement Phase 1b-1d credibility hardening by adding schema `v1.5` capability boundaries, introducing `S008` grounding warnings, upgrading showcase profiles to capability-aware configs, regenerating live showcase artifacts, and adding a visible showcase disclaimer when ungrounded claims still appear in generated text.
- Why: Credibility feedback highlighted ungrounded action claims as the highest-priority trust risk. The schema/compiler now encode capability limits directly, validator warns on unsupported action claims in profile policy, and regenerated showcase artifacts reflect capability-boundary prompts with explicit transparency for residual model non-compliance.
- Files: `packages/core/src/types.ts`, `packages/core/src/index.ts`, `packages/core/src/profile/merge.ts`, `packages/core/src/validator/schema.ts`, `packages/core/src/validator/safety.ts`, `packages/core/src/validator/engine.ts`, `packages/core/src/validator/format.ts`, `packages/core/src/compiler/engine.ts`, `packages/core/test/validator.test.js`, `packages/core/test/schema-format.test.js`, `packages/core/test/compile.test.js`, `profiles/haven.yaml`, `profiles/resolve.yaml`, `profiles/architect.yaml`, `docs/site/schema-reference.md`, `docs/site/index.md`, `docs/site/components/ShowcasePage.vue`, `experiment/evaluation/runs/2026-02-15-showcase-haven.json`, `experiment/evaluation/runs/2026-02-15-showcase-resolve.json`, `experiment/evaluation/runs/2026-02-15-showcase-architect.json`, `docs/site/data/showcase.json`, `docs/planning/development-memory.md`
- Follow-up: Start Phase 2 doc-completeness work (guides + integration recipes + API reference) now that Phase 1 is implemented and showcase artifacts are regenerated.

- Date: 2026-02-15
- Decision: Start Phase 2 with a concrete integration recipes guide and wire it into docs navigation/landing links.
...
- Date: 2026-02-15
- Decision: Implement Phase 4a array-extends composition for schema `v1.6`, including left-to-right parent-chain merge and S006 evaluation against the fully merged parent chain.
- Why: Organizational composition requires layering multiple reusable policy bases (brand/domain/channel) while preserving deterministic merge semantics and inheritance safety regression checks.
- Files: `packages/core/src/types.ts`, `packages/core/src/profile/extends.ts`, `packages/core/src/validator/engine.ts`, `packages/core/src/validator/schema.ts`, `packages/core/test/extends.test.js`, `packages/core/test/validator.test.js`, `packages/core/test/schema-format.test.js`, `profiles/test-fixtures/_extends-array-base.yaml`, `profiles/test-fixtures/_extends-array-domain.yaml`, `profiles/test-fixtures/_extends-array-channel.yaml`, `profiles/test-fixtures/_extends-array-child.yaml`, `profiles/test-fixtures/_extends-array-removal-test.yaml`, `docs/site/schema-reference.md`, `docs/site/guides/extending-profiles.md`, `docs/planning/development-memory.md`
- Follow-up: Implement Phase 4b locked constraints with S006 hard-error semantics when a child attempts to remove locked inherited rules.

- Date: 2026-02-15
- Decision: Implement Phase 4b locked constraints by supporting rule-object entries (`{ rule, locked? }`) in `behavioral_rules` and `capabilities.constraints`, preserving locked rules during merge/removal, and enforcing S006 hard errors on locked-rule removal attempts.
...
- Date: 2026-02-15
- Decision: Implement the landing-page/showcase redesign Phase 1 as a custom Vue-driven docs homepage and ship a new `/playground` experience with per-profile scenario flows, query-string state, and static response playback.
- Why: The previous landing page was structurally correct but low-conversion and the showcase compared profiles across mismatched vertical contexts. The redesign puts immediate product proof on the surface (Define -> Validate -> Compile narrative, profile cards, integration snippets) and replaces cross-vertical comparison with profile-specific playground flows.
- Files: `docs/.vitepress/config.mts`, `docs/.vitepress/theme/index.ts`, `docs/.vitepress/theme/Layout.vue`, `docs/.vitepress/theme/custom.css`, `docs/site/index.md`, `docs/site/components/LandingPage.vue`, `docs/site/playground.md`, `docs/site/components/PlaygroundPage.vue`, `docs/site/showcase.md`, `experiment/scripts/build-playground-data.mjs`, `docs/site/data/playground.json`, `package.json`, `docs/planning/development-memory.md`
- Follow-up: Add browser-native `@traits-dev/core` compilation wiring to the playground so slider changes use real `compileResolvedProfile`/`validateResolvedProfile` logic (currently prompt rendering is deterministic but not a full runtime compiler).

- Date: 2026-02-15
- Decision: Harden post-redesign credibility gaps by fixing static routing for `/playground`/`/showcase` via directory-index pages and expanding S008 action-claim scanning coverage to `identity.backstory` and `context_adaptations[].inject`.
